    ...
    public void compress(String inputFilePath) {
        ...
        for (int i = 0; i < img.size(); i++) {
            ...
            for (int j = 0;j<img.get(0).size();j++){
                red.get(i).add(Integer.valueOf(img.get(i).get(j).get(0)));
                green.get(i).add(Integer.valueOf(img.get(i).get(j).get(1)));
                blue.get(i).add(Integer.valueOf(img.get(i).get(j).get(2)));
            }
        }
